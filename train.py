...
def train_one_epoch(loader, model, criterion, optimizer, device, **kwargs):
    ...
    for i, batch in pbar:
        ...
        obj_loss.update(loss[2].item(), images.size(0))
        ...
        pbar.set_description(('%15s' + '%14s' + '%14.4g' * 6) % 
                                ...
                                total_loss.avg, box_loss.avg, cls_loss.avg, obj_loss.avg))
...
def train(opt, device):
    ...
    no_autoanchor = getattr(opt, 'no_autoanchor')
    ...
    train_loader, val_loader, train_dataset = build_dataloader(opt=opt)
    ...
    criterion = build_criterion(model=model, device=device, imgsz=img_size)
    ...
    if not no_autoanchor:
        check_anchors(dataset=train_dataset, model=model, imgsz=img_size) # run AutoAnchor
    ...
    obj_loss = AverageMeter('ObjLoss', ':5.4f')
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        train_one_epoch(loader=train_loader, model=model, criterion=criterion, optimizer=optimizer, 
                        ...
                        lf=lf, momentum=momentum, epoch_time=epoch_time, batch_time=batch_time,
                        total_loss=total_loss, box_loss=box_loss, cls_loss=cls_loss, obj_loss=obj_loss)
