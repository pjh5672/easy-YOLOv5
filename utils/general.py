...
def non_max_suppression(preds, conf_thres, nms_thres, multi_label=False, agnostic=False, max_det=300):
    ...
    xc = preds[..., 4] > conf_thres  # candidates
    ...
    for xi, x in enumerate(preds):  # image index, image inference
        ...
        # Box
        box = xcycwhn_to_x1y1x2y2(x[:, :4], w=1, h=1)  # center_x, center_y, width, height) to (x1, y1, x2, y2)

        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((j[:, None].float(), box[i], x[i, 5 + j, None]), 1)
