...
class Dataset:
    ...
    def __getitem__(self, index):
        ...
        label_ = np.concatenate((cls[:, None], boxes), axis=1)
        label = torch.zeros(max(len(label_), 1), 6)
        if len(label_):
            label[:, 1:] = torch.from_numpy(label_)
        else:
            label[:, 1] = -1
    ...
    def collate_fn(batch):
        ...
        return torch.stack(images, dim=0), torch.cat(labels, dim=0), filenames, shapes
